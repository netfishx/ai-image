      ...
      <DialogContent className="bg-foreground text-background">
        <DialogHeader>
          ...
          <div className="text-xs *:text-start">
            ...
            <p>您的专属链接：https://tuoyi.net?inviteCode={username}</p>
          </div>
          <div className="flex justify-between p-8">
            ...
            <Button
              ...
              onClick={() => {
                copyToClipboard(`https://tuoyi.net?inviteCode=${username}`);
                toast.success("复制成功");
              }}
            >
              复制链接
...
export function IdDialog({ username }: { username: string }) {
  const [open, setOpen] = useAtom(idDialogAtom);
  const [, copyToClipboard] = useCopyToClipboard();
  const qrCodeRef = useRef<HTMLDivElement>(null);
